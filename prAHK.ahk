...
FileReadLine, efff4, %path%, 7
FileReadLine, effblh, %path%, 8

FileReadLine, eff1, %path%, 9
FileReadLine, eff2, %path%, 10
FileReadLine, eff3, %path%, 11
FileReadLine, eff4, %path%, 12
FileReadLine, eff5, %path%, 13
; MsgBox, %eff1%
...
; ctrl alt shift 5 .....Application > Window > Eff Controls (Default is SHIFT 5)
; ctrl alt shift 7 .....Application > Window > Effs  (NOT the Eff Controls panel!) (Default is SHIFT 7)
...
;Therefore, we must start with a clean slate. For that, I chose the EFFS panel. Sending its focus shortcut multiple times, has no ill effs.

Sendinput, ^!+7 ;bring focus to the effs panel... OR, if any panel had been maximized (using the `~ key by default) this will unmaximize that panel, but sadly, that panel will still be the one in focus.
;Note that if the effs panel is ALREADY maximized, then sending the shortcut to switch to it will NOT un-maximize it. This is OK, though, because I never maximize the Effs Panel. If you do, then you might want to switch to the Eff Controls panel first, and THEN the Effs panel after this line.
...
Sendinput, ^!+7 ;Bring focus to the effs panel AGAIN. Just in case some panel somewhere was maximized, THIS will now guarantee that the Effs panel is ACTAULLY in focus.
...
{
	;MsgBox,%item%
;******FUNCTION FOR DIRECTLY APPLYING A PRESET EFF TO A CLIP!******
...
;NOTE: I built this under the assumption that your Effs Panel will be on the same monitor as your timeline. I can't guarantee that it'll work if the Effs Panel is on another monitor.
...
prFocus("effs") ;Brings focus to the effs panel. You must find, then copy/paste the prFocus() function definition into your own .ahk script as well. ALTERNATIVELY, if you don't want to do that, you can delete this line, and "comment in" the 3 lines below:

;Sendinput, ^+!7 ;CTRL SHIFT ALT 7 --- In Premiere's Keyboard Shortcuts panel, you nust find the "Effs" panel and assign the shortcut CTRL SHIFT ALT 7 to it. (The default shortcut is SHIFT 7. Because Premiere does allow multiple shortcuts per command, you can keep SHIFT 7 as well, or you can delete it. I have deleted it.)
;sleep 12
...
;The Effs panel's find box should now be activated.
...
loop
	...
	if (waiting2 > 40)
		{
		tooltip, 失败 - 没有找到插入符号. `n如果你的鼠标不能移动`, 按住键盘上的F1不松手，并且按下字母R键来重启脚本`n`n有可能是Premiere在错误的时机启动了自动保存。`n或者您未按照教程修改Premiere快捷键。`n您可以联络paomao@paomao.cc获取帮助。
...
;;;and fortunately, AHK knows the exact X and Y position of this caret. So therefore, we can find the effs panel find box, no matter what monitor it is on, with 100% consistency!
...
global path
; 从配置文件中读取值
FileReadLine, xValue, %path%, 1
FileReadLine, yValue, %path%, 2
FileReadLine, debug, %path%, 3
;MsgBox, %xValue%
; 将值转换为整数
xValue := Trim(xValue)
yValue := Trim(yValue)

;MsgBox, xValue: %xValue%`nyValue: %yValue%
...
;if (debug){
;	msgbox, 鼠标应该在放大镜的中间。 ;;<--comment this in for help with debugging.
;}
...
MouseMove, %xValue%, %yValue%, 0, R ;----------------------(for testing)
...
if (debug){
	msgbox, 此刻鼠标应该定位在效果面板的第一个预设上 `n如果没有，说明配置有问题，请修改配置`n`n需要更多帮助请查看教程！按回车键继续
}
...
;;At this point in the function, I used to use the line "MouseClickDrag, Left, , , %xposP%, %yposP%, 0" to drag the preset back onto the clip on the timeline. HOWEVER, because of a Premiere bug (which may or may not still exist) involving the duplicated displaying of single presets (in the wrong positions) I have to click on the Effs panel AGAIN, which will "fix" it, bringing it back to normal.
...
MouseMove, www/4, hhh/2, 0, R ;-----------------moves to roughly the CENTER of the Effs panel. Clicking here will clear the displayed presets from any duplication errors. VERY important. Without this, the script fails 20% of the time. This is also where the script can go wrong, by trying to do this on the timeline, meaning it didn't get the Effs panel window information as it should have.
sleep 5
...
MouseClickDrag, Left, , , %xposP%, %yposP%, 0 ;---clicks the left button down, drags this eff to the cursor's pervious coordinates and releases the left mouse button, which should be above a clip, on the TIMELINE panel.
sleep 5
...
; 窗口 > 选择查找框------- CTRL B
; ctrl alt shift 7 .....应用程序 > 窗口 > 效果  (NOT the Eff Controls panel!) (Default is SHIFT 7)